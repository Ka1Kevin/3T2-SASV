...
#SBATCH --job-name=proto_mfa #job name
#SBATCH --gpus=1
...
# TRIAL_NAME=("hp_3k6_ratio_modular_s2" "hp_5w4_ratio_modular_s2" "3w6_ratio_modular_o_hp_s2")
# trial_name=${TRIAL_NAME[$SLURM_ARRAY_TASK_ID]}
# set -x
. ./path.sh || exit 1
...
ASVspoof2019LA_dir=/export/fs05/arts/dataset/ASVspoof2019/LA/
data=data/ASVspoof2019LA
# data=data/hard_pair/ASVspoof5/$trial_ratio # data folder

trial=/export/fs05/ktan17/dataset/ASVSpoof2019LA/train_trial.txt
data_type="raw"  # shard/raw
config=conf/MFA_Conformer.yaml #wespeaker version
# config=conf/MHFA_wav2vec2.yaml
exp_dir=exp/proto_mfa
model_init=/export/fs05/ktan17/SASV2_Baseline/pretrained_weight/MFA_Conformer_stage_1.model
gpus="[0]"
checkpoint=
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  ./local/prepare_asvspoof2019LA.sh ${ASVspoof2019LA_dir} ${data} ${trial}
fi
