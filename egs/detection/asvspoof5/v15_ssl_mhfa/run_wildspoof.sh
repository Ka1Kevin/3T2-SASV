...
wildspoof_dir=/export/fs06/arts/dataset/wildspoof_test
# data=data/ASVspoof5_18w # data folder
# data=data/hard_pair/ASVspoof5/170554/
data=data/wildspoof_test
trial=/export/fs06/arts/dataset/wildspoof_test/protocol_test_label.tsv
data_type="raw"  # shard/raw
num_ref_utts=1
config=conf/SKA_TDNN.yaml #wespeaker version
# config=conf/MHFA_wav2vec2.yaml
# exp_dir=exp/softmax/stable_lr/lr_${initial_lr}
exp_dir=exp/ska_tdnn_wild_spoof_asvspoof5 #1372053
...
model_init=/export/fs05/ktan17/SASV2_Baseline/pretrained_weight/ska_tdnn_stage_1.model
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  ./local/prepare_wildspoof.sh ${wildspoof_dir} ${data} ${trial}
fi
...
avg_model=$exp_dir/models/best_model.pt
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
  ...
  cut -f2 -d" " /export/fs05/ktan17/ssl/wedefense-main/egs/detection/asvspoof5/v15_ssl_mhfa/data/hard_pair/ASVspoof5/ratio111/s2/train/utt2lab | sort | uniq -c | awk '{print $2 " " $1}' > /export/fs05/ktan17/ssl/wedefense-main/egs/detection/asvspoof5/v15_ssl_mhfa/data/hard_pair/ASVspoof5/ratio111/s2/train/lab2num_utts
  for dset in eval; do
  # for dset in train; do
      echo $dset
      python wedefense/bin/logits_to_llr_new.py \
	  --logits_scp_path ${exp_dir}/posteriors/$dset/logits.scp \
	  --train_label /export/fs05/ktan17/ssl/wedefense-main/egs/detection/asvspoof5/v15_ssl_mhfa/data/hard_pair/ASVspoof5/ratio111/s2/train/utt2lab 
