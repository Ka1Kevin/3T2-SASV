...
def main(model_path,
    ...
    if (configs['model_args']['embed_dim'] < 0):  # TODO check
        ...
        if fusion_type == 'feat_cat':
            configs['projection_args']['embed_dim'] = base_dim * (N_refs+1)
        elif fusion_type in ['xattn_add', 'time_cat']:
            configs['projection_args']['embed_dim'] = base_dim
        else:
            raise ValueError(f"Unknown fusion_type: {fusion_type}")
        # configs['projection_args']['embed_dim'] = configs['model_args']['embed_dim'] # 256
    
    configs['projection_args']['num_class'] = num_classes # 3
    configs['projection_args']['do_lm'] = configs.get('do_lm', False) # False
    ...
    with torch.no_grad():
        output = projection(torch.from_numpy(embd),
                            torch.from_numpy(np.zeros(embd.shape[0]))) # (282456, 3)
