...
from wedefense.utils.executor_new import train_epoch, val_epoch
...
def train(config='conf/config.yaml', **kwargs):
    ...
    train_utt_lab_list = read_table(train_label) # 读取文件，格式为["spk#utt", "label"]
    ...
    if configs.get('val_label'):
        ...
        with open(val_data, "r") as f:
            all_val_lines = [line.strip() for line in f]
        total_val = len(all_val_lines)
        if rank == 0:
            logger.info(f"[Validation] Total val samples before sampling = {total_val}")

        subset_size = configs.get("val_sample_num", 5000)

        if subset_size < total_val:
            import random
            random.seed(configs['seed'])
            sampled_lines = random.sample(all_val_lines, subset_size)
        else:
            sampled_lines = all_val_lines

        sampled_raw_list = os.path.join(configs['exp_dir'], "val_raw_sampled.list")
        with open(sampled_raw_list, "w") as f:
            for line in sampled_lines:
                f.write(line + "\n")

        sampled_utt2lab = os.path.join(configs['exp_dir'], "val_utt2lab_sampled")
        with open(sampled_utt2lab, "w") as f:
            for wav_path in sampled_lines:
                utt = os.path.basename(wav_path).replace(".flac", "")
                for u, lab in val_utt_lab_list:
                    if u == utt:
                        f.write(f"{u} {lab}\n")
                        break

        if rank == 0:
            logger.info(f"[Validation] Using subset = {len(sampled_lines)} samples")

        val_data = sampled_raw_list
        val_label = sampled_utt2lab

        val_iter = max(1, len(sampled_lines) // world_size // batch_size)

        val_dur_file = os.path.join(os.path.dirname(val_label), 'utt2dur')
        val_dataset = Dataset(
            configs['data_type'],
            val_data,
            configs['dataset_args'],
        ...
        if rank == 0:
            logger.info(f"[Validation] Iter per epoch = {val_iter}")
