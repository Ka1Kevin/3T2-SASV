...
class Bottle2neck(nn.Module):
    ...
    def forward(self, x):
        ...
        sp = spx[0]
        # idx=0: 先初始化 out_cat 为 Tensor
        sp = spx[0]
        sp = self.convs[0](sp)   # literal index OK
        sp = self.relu(sp)
        sp = self.bns[0](sp)     # literal index OK
        out_cat = sp
        idx = 0
        first = True
        ...
        for conv, bn in zip(self.convs, self.bns):
            if first:
                ...
                idx += 1
                sp = sp + spx[idx]
                sp = conv(sp)       # ✅ conv 是 module（不是切片出来的 list 元素）
                sp = self.relu(sp)
                sp = bn(sp)
                out_cat = torch.cat((out_cat, sp), 1)
        
        out = torch.cat((out_cat, spx[self.nums]),1)
        out = self.conv3(out)
        out = self.relu(out)
        out = self.bn3(out)
        out = self.se(out)
...
class fwSKAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        U = torch.stack(conv_outs, dim=0).sum(dim=0).permute(0, 2, 3, 1)    # Fuse: B, C, F, T -> B, F, T, C
...
class cwSKAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        U = torch.stack(conv_outs, dim=0).sum(dim=0)        # Fuse: B, C, F, T
...
class SKA_TDNN_net(nn.Module):
    ...
    def forward(self, x, aug: bool = True):
        with torch.no_grad():
...
def SKA_TDNN(eca_c=1024, eca_s=8, log_input=True, num_mels=80, num_out=192, **kwargs):
    model = SKA_TDNN_net(block=Bottle2neck, C=1024, model_scale=eca_s, log_input=log_input, num_mels=num_mels, num_out=num_out, resblock=ResBlock, **kwargs)
    return model