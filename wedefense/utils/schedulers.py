...
import math
from wedefense.utils.cosine_annealing_warmup_restarts import CosineAnnealingWarmupRestarts
...
class _CompatScheduler:
    """
    适配器：对外暴露 get_lr() -> float（给日志/打印用）
    内部真正的 step() 仍调用底层调度器的 get_lr() -> list（不被破坏）
    """
    ...
    def step(self, epoch=None):
        return self._inner.step(epoch)

    # 让日志/print调用变成返回单个 float，而不是 list
    def get_lr(self):
        return float(self.optimizer.param_groups[0]['lr'])

    # 需要时，外部还可以拿到底层的 list（可选）
...
def CosineWarmupScheduler(optimizer, num_epochs, epoch_iter,
                          initial_lr, final_lr,
                          warm_up_epoch=5,
                          cycle_mult=1.0,
                          gamma=0.8,
                          **kwargs):
    total_steps = num_epochs * epoch_iter
    warmup_steps = int(warm_up_epoch * epoch_iter)

    scheduler = CosineAnnealingWarmupRestarts(
        optimizer,
        first_cycle_steps=total_steps,  # or one cycle
    ...
    return _CompatScheduler(scheduler)
