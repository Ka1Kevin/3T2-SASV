...
for suffix in $(seq 0 $(($nj - 1))); do
  ...
  CUDA_VISIBLE_DEVICES=${gpus[$idx]} python -u wedefense/bin/extract_MFA.py \
...
if [ $embed_num -eq $wavs_num ]; then
  ...
  echo "Failed to extract embedding for ${store_dir}, the embed num is ${embed_num}, the wavs num is ${wavs_num}" | tee ${embed_dir}/extract.result
fi
